...
(defn $eval
  ...
  (cond
    ...
    (case (first expr)
      ...
      do
      (if (empty? (second expr))
        nil
        (last (map #($eval % env) (rest expr))))

      dotimes
      (let [times ($eval (second (second expr)) env)
      ...
      (let [var (first (second expr))
            value ($eval (second (second expr)) env)
            body (third expr)
            ]
        ($eval body (assoc env var value)))
      ...
      (let [args (rest expr)]
        (loop [newArgs args
          ...
          (cond
            (not (boolean? ($eval test env))) ($eval exp env)
            (true? test) ($eval exp env)
            (empty? newArgs) nil
            :else
            (if (true? ($eval test env))
              ($eval exp env)
              (recur (drop 2 newArgs) (first newArgs) (second newArgs))
              )
            )
          )
        )
...
(with-out-str ($eval '(dotimes (x 0)
                        (println x))
                     {'println println})) ) )
  (is (= "0123456789" (with-out-str ($eval '(dotimes (x 10)
                        (pr x))
                     {'pr pr})) ) )
  (is (= (let [nl (System/lineSeparator)]
